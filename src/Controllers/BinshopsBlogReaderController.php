...
class BinshopsBlogReaderController extends Controller
{
     ...
     * @return mixed
     */
    public function indexLang($languageCode, $category_slug = null)
    {
        // the published_at + is_published are handled by BinshopsBlogPublishedScope, and don't take effect if the logged in user can manageb log posts
        $title = 'Blog Page'; // default title...
        ...
        $posts = $posts->whereHas("language", function ($q) use($languageCode){
                $q->where("code", $languageCode);
            })->where('is_published', '=', 1)->where('posted_at', '<', Carbon::now()->format('Y-m-d H:i:s'))->orderBy("posted_at", "desc")->paginate(config("binshopsblog.per_page", 10));
     ...
     * @throws \Exception
     */
    public function searchLang($languageCode, Request $request)
    {
        if (!config("binshopsblog.search.search_enabled")) {
            throw new \Exception("Search is disabled");
        }
